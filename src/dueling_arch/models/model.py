...
class Network(nn.Module):
    ...
    def __init__(self, in_size, h_size, n_actions):
        ...
        self.main = nn.Sequential(nn.Linear(in_size, h_size * 2), nn.ReLU(),
                                  nn.Linear(h_size * 2, h_size), nn.ReLU())

        # block in charge of state value estimation
        self.state_block = nn.Sequential(nn.Linear(h_size, 1))

        # block in charge of advantage value estimation
        self.adv_block = nn.Sequential(nn.Linear(h_size, n_actions))
